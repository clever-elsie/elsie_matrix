...
matrix<T>& matrix<T>::fma_impl_strassen(const matrix<U>&a,const matrix<V>&b){
  ...
  matrix<T> p1=matrix_mul<naive,base,false>(as[0],s1);
  matrix<T> p2=matrix_mul<naive,base,false>(s2,bs[3]);
  matrix<T> p3=matrix_mul<naive,base,false>(s3,bs[0]);
  matrix<T> p4=matrix_mul<naive,base,false>(as[3],s4);
  matrix<T> p5=matrix_mul<naive,base,false>(s5,s6);
  matrix<T> p6=matrix_mul<naive,base,false>(s7,s8);
  matrix<T> p7=matrix_mul<naive,base,false>(s9,sA);
